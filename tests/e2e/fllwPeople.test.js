...
const run = async () => {
	try {
		...
		const elementToLike = await page.$$('div[aria-label="Tambah Teman"]')
		if (elementToLike.length > 0) {
			const boundingBox = await elementToLike[0].boundingBox()
			let clicked = false
			do {
				try {
			...
			console.log('No element to ADD found')
		}

		// const countShare = await page.$$eval(
		// 	'div[aria-label="Tambah Teman"]',
		// 	element => element.length
		// )
		// if (countShare > 0) {
		// 	const selectorsShare = await page.$$('div[aria-label="Tambah Teman"]')
		// 	console.log(selectorsShare, 'ADD!')
		// 	await page.waitForTimeout(1000)
